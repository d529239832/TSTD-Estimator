...
from pyspark.sql.functions import col, when, round as spark_round, lit, rand
...
def parse_prep_dataset_args(argv: list[str]):
    ...
    try:
        opts, args = getopt.getopt(
            argv,
            "hd:t:x:l:r:c:y:n:s:f:p:g:w", # 第二个参数是一个字符串，表示短选项，每个字母后面的冒号表示该选项需要接受一个值。
...
def run_prep_dataset_processes(exp_args: list):
    ...
    spark = (
        ...
        .config("spark.driver.memory", "70g")      # 驱动程序内存
    ...
    if exp_args["normalize"]:
        ...
        #     # **means_dict 将字典 means_dict 中的内容展开，并将其添加到另一个字典中
        #     # ===== 如果是 length，额外记录 norm 映射 =====
        #     if i == "length" and exp_args["conditions"] and "length" in exp_args["conditions"]:
        #         length_norm_map = {}
        #
        #         for l in exp_args["conditions"]["length"]:
        #             # MinMaxScaler 的定义
        #             normed = (l - imin) / (imax - imin) if imax > imin else 0.0
        #             length_norm_map[str(l)] = round(normed, 3)
        #
        #         means_dict["length_norm_map"] = length_norm_map

        # 根据 最小 64B 最大 65535B 最大最小归一化
        FIXED_LENGTH_MIN = 172.0
        FIXED_LENGTH_MAX = 10320.0

        for i in exp_args["normalize"]:
            if i == "length":
                imin = FIXED_LENGTH_MIN
                imax = FIXED_LENGTH_MAX

                # 计算归一化并保留6位小数
                # 公式: round((x - min) / (max - min), 6)
                # 加上 when().otherwise() 是为了防止数据超出 64-65535 范围时出现负数或大于1的值
                df = df.withColumn(
                    "length_normed",
                    spark_round(
                        (when(col("length") < FIXED_LENGTH_MIN, FIXED_LENGTH_MIN)
                        .when(col("length") > FIXED_LENGTH_MAX, FIXED_LENGTH_MAX)
                        .otherwise(col("length"))
                        - FIXED_LENGTH_MIN) / (FIXED_LENGTH_MAX - FIXED_LENGTH_MIN),3)) # 点后保留几位

            # 更新元数据字典
            means_dict[i] = {"min": imin, "max": imax}

            # 处理映射表 length_norm_map，这里也要保留6位
            if i == "length" and exp_args["conditions"] and "length" in exp_args["conditions"]:
                length_norm_map = {}
                for l in exp_args["conditions"]["length"]:
                    # 使用 Python 的 round 函数
                    normed = (l - imin) / (imax - imin) if imax > imin else 0.0
                    length_norm_map[str(l)] = round(float(normed), 3)  # 点后保留几位
                means_dict["length_norm_map"] = length_norm_map
