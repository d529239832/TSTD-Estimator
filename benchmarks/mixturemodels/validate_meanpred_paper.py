...
def run_validate_pred_processes_mean(exp_args: list):
    ...
    for idx, cond_dict in enumerate(conditions):
        ...
        if exp_args.get("plotsample", False):
            cond_df_sample = cond_df.sample(fraction=0.0525, seed=idx * 101012)
            #cond_df_sample = cond_df.sample(fraction=0.002, seed=idx * 101012)
        ...
        for model_list in exp_args["models"]:
            ...
            if exp_args["plottail"]:
                ...
                if idx == 0:
                    ...
                    ax.set_xlim(0, 180)  # 设置X轴范围
                    ax.set_ylim(1e-6, 1)
                    ax.set_title("'length': 10,320" + " bytes", fontsize=25)
